...
%      \caption{\( n\_issues = 1,  \sigma = 0.1\) }
 ...
 %      \caption{\(n\_issues = 1, \sigma = 0.02\) }
  ...
  %     \caption{\(n\_issues = 7, \sigma = 0.1\)}
    ...
    show that after it's bigger than 0.1 \(\sigma\) has the same impact on the
    system. Therefore we restrict our following analysis to the \((0.0, 0.1 ] \)
    range. The same can be inferred from \(n> 5\). And the effect of \(\rho\) is
    the expected: the bigger the noise more dispersed is the final state of the
    system.
