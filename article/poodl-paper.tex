...
\textcolor{blue!60}{ The \(\Delta_{ij} \) term is equal to \( o_{i,k} (t) - o_{j,k}
  (t)\). As mentioned, we also test cases in which it's equal to \( x_i(t) -
  x_j(t) \) and to \(x_{i}(t) - o_{j,k}(t)\). \(p\), for its part, is a global
  parameter  used to model the likelihood of the
  other agent's (\(j\)) opinion being true \cite{martins12b}.}

  \textcolor{blue!60}{ Furthermore, there is the noise: we draw another agent
    \(i\) whose opinion \( o_{i,k}(t+1)\) is equal to \(o_{i,k}(t) + r\) where
    \(r\) is taken from a Normal distribution of mean 0 and standard deviation
    \(\rho\). \(\rho\) is then a global parameter of the simulation (\( 0.0 <
    \rho \leq 0.1\)). From a theoretical point of view the noise is justified as
    a way of accounting for the effect of factors not related to social
    influence that make the agents change their opinion about issues
    ...
    o_{i,k}(t+1) = 1\). Likewise, if \(o_{i,k}(t) + r < 0 \) then \(
    o_{i,k}(t+1) = 0\).}

  \section{Model Results}
  % To analyze the impact of the parameters we start the analysis with a general
  \begin{itemize}
  \item ( \(0.1 \leq p \leq 0.99 \))
  \item \( 0.0 \leq
  \item \textit{p$\_$intran} \leq 0.1 \)
  \item \( 1 \leq n \leq 10 \)
  \item \( 0.0 < o < 1.0 \)
  \item \( 500 < N < 5000\)
  \item  \( 0.01 \leq \sigma \leq 0.5\)
  \end{itemize}
